...
class TitleActionColumn extends \yii\grid\DataColumn
{
    ...
    protected function initDefaultButtons()
    {
        if (!isset($this->buttons['view'])) {
            $this->buttons['view'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('miranda', 'View'),
                    'aria-label' => Yii::t('miranda', 'View'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return Html::a(Yii::t('miranda', 'View'), $url, $options);
            };
        }
        if (!isset($this->buttons['update'])) {
            $this->buttons['update'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('miranda', 'Edit'),
                    'aria-label' => Yii::t('miranda', 'Edit'),
                    'data-pjax' => '0',
                ], $this->buttonOptions);
                return Html::a(Yii::t('miranda', 'Edit'), $url, $options);
            };
        }
        if (!isset($this->buttons['delete'])) {
            $this->buttons['delete'] = function ($url, $model, $key) {
                $options = array_merge([
                    'title' => Yii::t('miranda', 'Delete'),
                    'aria-label' => Yii::t('miranda', 'Delete'),
                    'data-confirm' => Yii::t('yii', 'Are you sure you want to delete this item?'),
                    'data-method' => 'post',
                    'data-pjax' => '0',
                ], $this->buttonOptions);

                return Html::a(Yii::t('miranda', 'Delete'), $url, $options);
            };
        }
    }
     ...
     * @param \common\db\ActiveRecord $model the data model
